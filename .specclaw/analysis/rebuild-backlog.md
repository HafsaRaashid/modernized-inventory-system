...
**Status notes (human-added):**
- BUILT: change `asset-assignment`, merged to `master` at commit `d454775` (branch `specclaw/asset-assignment`), 2026-08-30. All 5 tasks complete, verify PASS on all 18 acceptance criteria (`.specclaw/changes/asset-assignment/verify-report.md`), 74 backend + 104 frontend tests passing. New `AssetAssignmentsController` (a POST issuing a fixed asset to a room and a GET listing a room's current assignments), implementing the composite insert-plus-decrement as ONE atomic `SaveChangesAsync()` call rather than an explicit DB transaction (the EF Core InMemory test provider doesn't support explicit transactions, and one call is already atomic). CQ-026's guard-bypass gap is closed structurally: `FixedAsset.Quantity` can only be decremented through this one guarded endpoint. Adopts CQ-028's proposed default (single atomic operation) as its implementation; CQ-028 itself remains formally open. The Asset Assignment screen (reached from Main Menu, not admin-gated) wires up `/asset-assignment`, which previously 404'd. Still open: CQ-027/CQ-028 (unanswered, non-blocking), UI screenshot sign-off (deferred to end of backlog), an own-documented assumption about which responsibility row wins when a room has more than one (most-recently-created), and fresh GM fixtures for the rebuild's own new non-negative guard (legacy's GM-040 only pins the unguarded legacy behavior).
